 ...
 */
require_once '../clase/administracionGeograficaClass.php';

$Obj_Geografia = new Geografia();

$tipo = $_POST["tipo"];

if ($tipo == 1) {
    $retorno = $Obj_Geografia->verPaises($_POST);
    echo $retorno;
}if ($tipo == 2) {
    $retorno = $Obj_Geografia->agregarPaises($_POST);
    echo $retorno;
}if ($tipo == 3) {
    $retorno = $Obj_Geografia->consultarIdPais($_POST);
    echo $retorno;
}if ($tipo == 4) {
    $retorno = $Obj_Geografia->agregarEstado($_POST);
    echo $retorno;
}
// if ($tipo == 5) {
//     $retorno = $Obj_Geografia->agregarEstado($_POST);
...
// }
if ($tipo == 5) {
    $retorno = $Obj_Geografia->verEstados($_POST);
    echo $retorno;
}if ($tipo == 6) {
    $retorno = $Obj_Geografia->verCiudad($_POST);
    echo $retorno;
}
//if ($tipo == 2) {
