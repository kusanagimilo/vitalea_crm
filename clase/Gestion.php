...
LEFT join tipificacion as D on D.id = A.tipificacion_id");
    ...
    public function obtenerExamen2($categoria_id, $id_plan) {

        $query = $this->conexion->prepare("select plai.id_item,plai.id_plan_item,chq.nombre 
from plan_item plai 
inner join examen chq on chq.id = plai.id_item
where plai.id_plan = :id_plan
and plai.tipo_item = 'chequeo'
and chq.grupo_id = :categoria_id");
        ...
        $rows = $query->fetchAll(PDO::FETCH_ASSOC);
        foreach ($rows as $valor) {
            $opciones[] = array('id' => $valor["id_item"], 'nombre' => $valor["nombre"]);
        }
        return json_encode($opciones);
    }

    public function examen_precio_no_perfil2($examen_id, $id_plan) {
        $query = $this->conexion->prepare("SELECT precio_plan FROM plan_item WHERE id_plan = :id_plan AND tipo_item = 'examen' AND id_item = :examen_id");
        $query->execute(array(':examen_id' => $examen_id,
            ':id_plan' => $id_plan));
        $rows = $query->fetchAll();
        foreach ($rows as $valor) {
            $precio = $valor["precio_plan"];
        }
